...
from .controllers import checkout, order_details, orders, recipes, resources, sandwiches
...
app = FastAPI(
    title="Sandwich Shop Ordering API",
    description=(
        "CRUD management for sandwiches, resources, recipes, orders, and "
        "order details, plus inventory-aware checkout."
    ),
    version="1.0.0",
)
...
def read_root():
    return {
        "name": "Sandwich Shop Ordering API",
        "documentation": "/docs",
        "status": "running",
    }
...
def read_order_summary(order_id: int, db: Session = Depends(get_db)):
    order = orders.read_one(db, order_id=order_id)
    if order is None:
        raise HTTPException(status_code=404, detail="Order not found")
    return checkout.summarize(order)
...
def checkout_order(order_id: int, db: Session = Depends(get_db)):
    ...
    if order is None:
        raise HTTPException(status_code=404, detail="Order not found")
    return checkout.complete(db, order)


@app.post("/sandwiches/", response_model=schemas.Sandwich, tags=["Sandwiches"])
def create_sandwich(
    sandwich: schemas.SandwichCreate,
    db: Session = Depends(get_db),
):
...
@app.get(
    ...
    response_model=schemas.Sandwich,
    tags=["Sandwiches"],
)
...
def create_recipe(recipe: schemas.RecipeCreate, db: Session = Depends(get_db)):
    if sandwiches.read_one(db, sandwich_id=recipe.sandwich_id) is None:
        raise HTTPException(status_code=404, detail="Sandwich not found")
    if resources.read_one(db, resource_id=recipe.resource_id) is None:
        raise HTTPException(status_code=404, detail="Resource not found")
...
def create_order_detail(
    ...
    if orders.read_one(db, order_id=order_detail.order_id) is None:
        raise HTTPException(status_code=404, detail="Order not found")
    if sandwiches.read_one(db, sandwich_id=order_detail.sandwich_id) is None:
        raise HTTPException(status_code=404, detail="Sandwich not found")
